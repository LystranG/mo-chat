...
public final class CallOfflineNotificationMqConsumer {
    ...
    /**
     * 启动 RocketMQ 消费者，订阅离线通知 topic。
     * <p>生命周期由 {@code CallServiceRuntimeLifecycle} 统一管理，不使用 @PostConstruct。</p>
     */
    ...
    /**
     * 停止 RocketMQ 消费者。
     * <p>生命周期由 {@code CallServiceRuntimeLifecycle} 统一管理，不使用 @PreDestroy。</p>
     */
    public void stop() {
        if (consumer != null) {
            consumer.shutdown();
