...
- Define client/server message IDs: client-provided `clientMsgId` for idempotency, server-generated Snowflake `msgId` for global identity, and per-conversation `seq` for history pagination.
- Add TLS 1.3 for client-server transport encryption; `mochat.tls.enabled=false` is rejected at startup, certificate/private-key overrides must be configured as a pair, both may stay empty only when `mochat.tls.self-signed=true`, and an explicit valid pair takes precedence over self-signed generation.
...
- `data-model-and-history-pagination`: PostgreSQL schema for user/friend/group relations and `seq`-based history (`cursorSeq` or `startSeq/endSeq`, two modes are mutually exclusive, `limit<=50`).
...
- Operations impact: requires local environment profile (Docker Compose recommended) for Redis/PostgreSQL/RocketMQ and a TLS certificate provisioning workflow (TLS is mandatory; either generated self-signed when both override paths are empty and `self-signed=true`, or an operator-supplied certificate/private-key pair provided together).
