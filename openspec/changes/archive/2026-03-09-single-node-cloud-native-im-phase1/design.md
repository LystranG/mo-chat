...
### 4) Security model
Decision:
- Enforce TLS 1.3 for all TCP links in phase-1; `mochat.tls.enabled=false` is invalid and must fail fast.
- Operators may either leave `mochat.tls.certificate-path` and `mochat.tls.private-key-path` both empty with `mochat.tls.self-signed=true` to auto-generate a self-signed certificate, or provide both files explicitly, in which case the explicit pair takes precedence.
...
Decision:
- Use per-conversation `seq` for `GET /history`: cursor mode uses `cursorSeq`, range mode uses inclusive `startSeq/endSeq`, the two modes are mutually exclusive, and `limit` defaults to 50 while being capped at 50.

Rationale:
- Per-conversation `seq` keeps history windows stable for large message volumes, avoids deep offset scans, and lets cursor/range queries share one bounded contract.
