...
`build-local-images.sh` 默认使用宿主机 native 编译语义。需要生成本地 k3s 可运行的 Linux native 容器镜像时，使用 `scripts/build-local-images.sh --docker-compile`；可用 `NATIVE_CONTAINER_MEMORY=12g` 调大 builder 容器内存。
